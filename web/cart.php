...
?>
    ...
    <noscript><img height="1" width="1" style="display:none"
            src="https://www.facebook.com/tr?id=<?= trim($FB_Pixel_Code) ?>&ev=PageView&noscript=1" /></noscript>
                                ...
                                <ul class="discount">
                                    ...
                                    ?>
                                            <li>
                                                ...
                                                <div class="input">
                                                    <input type="text" id="use_bonus" name="use_bonus" value="<?= $order_info['use_bonus'] ?>" max="<?= $member_info['bonus_num'] ?>" pattern="[0-9]">
                                                    <div class="remaining_point">總積點 <span><?= $member_info['bonus_num'] ?></span> ，本次消費可用 <span>300</span> 積點</div>
                                                </div>
                                            </li>
                                    <?
                                        }
...
-->
                                            ...
                                            <div class="full" <?= (($traffic_money > 0) ? " style='color:#797979;'" : "") ?>><?= $free_traffic_money_str ?></div>
                                        </div>
                                    </li>
                                </ul>
                                <ul class="total">
    ...
    <script>
        ...
        function form_submit() {
            ...
            */
            ...
            if (invoice_type_str == "紙本發票-三聯式") {
                /*
                     if($("input[name=FO_recipient_address_2]").size() > 0)
                     {
                */
                if ($("input[name=FO_recipient_address_2]").val() == "") {
                    $("#FO_recipient_address_alert_2").addClass("required");
                    error_sn = 1;
                    the_alert += "發票地址必填！\n";
                } else if ($("select[name=city_2]").val() == "") {
                    $("#FO_recipient_address_alert_2").addClass("required");
                    error_sn = 1;
                    the_alert += "發票地址必填！\n";
                } else if ($("select[name=area_2]").val() == "") {
                    $("#FO_recipient_address_alert_2").addClass("required");
                    error_sn = 1;
                    the_alert += "發票地址必填！\n";
                } else {
                    $("#FO_recipient_address_alert_2").removeClass("required");
                }
                //      }
            ...
            if (invoice_type_str == "紙本發票-二聯式") {
                ...
                */
                if ($("input[name=FO_recipient_address]").val() == "") {
                    $("#FO_recipient_address_alert").addClass("required");
                    error_sn = 1;
                    the_alert += "發票地址必填！\n";
                } else if ($("select[name=city_1]").val() == "") {
                    $("#FO_recipient_address_alert_2").addClass("required");
                    error_sn = 1;
                    the_alert += "發票地址必填！\n";
                } else if ($("select[name=area_1]").val() == "") {
                    $("#FO_recipient_address_alert_2").addClass("required");
                    error_sn = 1;
                    the_alert += "發票地址必填！\n";
                } else {
                    $("#FO_recipient_address_alert").removeClass("required");
                }
                //      }

            }

            if (invoice_type_str == "雲端發票") {
                /*
                     if($("input[name=FO_recipient_address]").size() > 0)
                     {
                */
